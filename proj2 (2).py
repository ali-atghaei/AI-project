...
min_room_space=int(raw_input("min space"))
max_room_space=int(raw_input("max space"))
...
def space(genome):
    space_score=0
    for i in range (0,len(genome),4):
        room_space=int((genome[i]-genome[i+2])*(genome[i+1]-genome[i+3]))
        if not room_space<min_room_space:
            space_score=space_score+1
        if not room_space>max_room_space:
            space_score=space_score+1
    return space_score

	
def eval_func(genome):
    return light_func(genome)+inrange_func(genome)+unusedplace_func(genome)+overlab_func(genome)+av_function(genome)+space(genome)
